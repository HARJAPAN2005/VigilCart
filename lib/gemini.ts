...
// ---------------------------------------------------------------------------
// Safety boundary — embedded in every Gemini prompt
// ---------------------------------------------------------------------------
...
/** Thrown when Gemini is unavailable (no key, network failure, etc.). */
...
/**
 * Use Gemini to parse a natural-language shopping request into a structured
 ...
 */
export async function compileIntentWithGemini(
  userText: string
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            maxBudgetINR: { type: Type.NUMBER },
            needByDate: { type: Type.STRING },
            mustBeReturnable: { type: Type.BOOLEAN },
          },
          required: ['item', 'maxBudgetINR', 'needByDate', 'mustBeReturnable'],
        },
      },
    }),
    timeout(TIMEOUT_MS),
...
/**
 * Use Gemini to generate a concise explanation of the guard run.
 * The deterministic guard result is passed as context — Gemini must never
 * alter pass/block results, scores, or approval gates.
 */
export async function explainRunWithGemini(
    ...
    client.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          required: ['explanation', 'merchantFlags', 'safetyNote'],
        },
      },
    }),
    timeout(TIMEOUT_MS),
...
function timeout(ms: number): Promise<never> {
  return new Promise((_, reject) =>
    setTimeout(() => reject(new GeminiUnavailableError(`Gemini request timed out after ${ms}ms`)), ms)
  );
}
